...
resource "aws_autoscaling_group" "mysql-asg" {
  vpc_zone_identifier = ["${aws_subnet.public.*.id}"]
  name = "tf-${var.StackName}-asg-${aws_launch_configuration.mysql-lc.id}"
  max_size = "${var.asg_max}"
  min_size = "${var.asg_min}"
  desired_capacity = "${var.asg_desired}"
  force_delete = true
  launch_configuration = "${aws_launch_configuration.mysql-lc.name}"
  load_balancers = ["${aws_elb.mysql-elb.name}"]
  ...
  tag {
    key = "Name"
    value = "tf-${var.StackName}-mysql-asg"
