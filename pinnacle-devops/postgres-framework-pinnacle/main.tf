...
resource "aws_autoscaling_group" "postgres-asg" {
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.postgres-lc.name}"
  load_balancers = ["${aws_elb.postgres-elb.name}"]
  ...
  tag {
    key = "Name"
    value = "tf-${var.StackName}-postgres-asg"
    propagate_at_launch = "true"
  }
}

resource "aws_launch_configuration" "postgres-lc" {
